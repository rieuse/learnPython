...
def get_detail():
    while True:
        url = pop_redis()
        # url = 'https://bbs.hupu.com/14577542.html'
        try:
            html = requests.get(url, headers=headers).content.decode('utf-8')
            title = ''.join(etree.HTML(html).xpath('//*[@id="j_data"]/@data-title'))
            content = ''.join(etree.HTML(html).xpath('//*[@class="quote-content"]/text()'))
            # reply = etree.HTML(html).xpath('//*[@class="quote-content"]/text()')
            reply = BeautifulSoup(html, 'html.parser').find_all('table', class_="case")
            # print(reply)
            comments = []
            for i in reply:
...
def thread_main():
    ...
    tasks = threadpool.makeRequests(get_detail, '')
